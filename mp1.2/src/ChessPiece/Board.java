...
public class Board extends JPanel {
	 ...
	 * 
	 */
	private static final long serialVersionUID = 6071833142546019276L;
	...
	public Board(boolean customized)
	{
		board = new Piece [8][8];
		...
		board[0][4] = new Queen(4,0,false); 
		board[0][3] = new King(3,0,false);
		
		if(customized){
			board[3][0] = new Bomb(0,3,false);
			board[3][4] = new Stone(4,3,false);
			board[4][7] = new Bomb(7,4,true);
			board[4][4] = new Stone(4,4,true);
		}
		...
		board[7][3] = new King(3,7,true); 
		board[7][4] = new Queen(4,7,true); 
	...
	/*Deep copy this board and return the board*/
	public Board copy(){
		Board ret = new Board(false);
		ret.clear();
		for(int i =0; i< 8; i++){
			for(int j =0; j<8; j++){
	...
	public void setPiece(int x, int y, String name, boolean owner){
		if( name.equals("Bishop")){
			...
			board[y][x] = new Pawn(x,y,owner,true);
		}
			...
			board[y][x] = new Stone(x,y,owner);
		}
		else if(name.equals("Bomb")){
			board[y][x] = new Bomb(x,y,owner);
		}
	}
	...
	public boolean validMoveOnBoard(int x, int y, int tX, int tY){
		...
		//System.out.println(board[y][x].isValidMove(tX, tY));
		if(board[y][x].isValidMove(tX, tY)){
			...
			if( board[y][x] instanceof Bishop )
				...
				int xDiff = Math.abs(thisPawn.getPosition().getFirst()-tX);
				int yDiff = tY-thisPawn.getPosition().getSecond();
			...
			else if(board[y][x] instanceof Rook ||board[y][x] instanceof Bomb )
				return checkInLine(x,y,tX,tY);
		}
		return false;
	}
	...
	public void updateMove(int x, int y, int targetX, int targetY){
		...
		if(board[y][x] instanceof King){
			boolean player = board[y][x].getOwnership();
			if(player)
				kingOne.setPair (targetX, targetY);
			else 
				kingTwo.setPair (targetX, targetY);
		}

		if((board[y][x] !=null && board[targetY][targetX] instanceof Bomb) 
			|| (board[y][x] instanceof Bomb && board[targetY][targetX] !=null)){
		...
		board[targetY][targetX].getPosition().setPair(targetX, targetY);
		
		board[y][x] = null;
	}

	/*Check whether the King will be captured*/
	...
	public boolean finalValidMove (int x, int y, int tx, int ty){
		...
		Piece other = board[ty][tx] == null? null:board[ty][tx].copy();
		Piece otherOne = board[y][x] == null? null:board[y][x].copy();;
		updateMove(x, y, tx, ty);
		boolean check = canGoToKing(ownership);
		
		board[y][x] = (otherOne == null)? null:otherOne.copy();
		board[ty][tx] = (other == null)? null:other.copy();
		return validMove && (!check);
	}
	
	/* This function help to check whether the ownership's king is in checkmate
						...
						for(int jj =0; jj<8;jj++){
							if(this.finalValidMove(j,i,jj,ii)){
								...
								if(!myCopy.canGoToKing(ownership)){
									//System.out.println((myCopy.board[0][7] instanceof Queen) + " " + ownership + " "+ j +" "+i +" "+jj + " "+ii);
									return false;
								}
							}
						}
	...
	public boolean checkmate (boolean ownership){
		//System.out.println(canGoToKing(ownership));
		//System.out.println(noMove(ownership));
