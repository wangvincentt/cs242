...
public class Network {
	...
	public boolean requestBroadcast(Writer report) {
		...
		do {
			try {
				currentNode.broadcastRefactor(report, this,"' accepts broadcast packet.\n");
				currentNode.broadcastRefactor(report, this,"' passes packet on.\n");
				report.flush();
			} catch (IOException exc) {
				// just ignore
			}
	...
	public boolean requestWorkstationPrintsDocument(String workstation,
		...
		try {
			firstNode.workstationRefactor("'",workstation, report);
			firstNode.workstationRefactor("' requests printing of '",document, report);
			firstNode.workstationRefactor("' on '",printer, report);
			report.write("' ...\n");
		} catch (IOException exc) {
			// just ignore
		}
		...
		try {
			startNode.workstationRefactor2(report, this);
			report.flush();
		} catch (IOException exc) {
			// just ignore
		}
		...
		while ((!packet.destination.equals(currentNode.name))
			...
			try {
				currentNode.workstationRefactor2(report, this);
				report.flush();
			} catch (IOException exc) {
				// just ignore
			}
	...
	public void writeAccountingReport(Writer report, String author, String title) throws IOException {
