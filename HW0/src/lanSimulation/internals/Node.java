...
public class Node {
		...
		if (type == Node.PRINTER) {
			try {
				if (document.message.startsWith("!PS")) {
					...
					network.writeAccountingReport(report, author, title);
				} else {
	...
	public void workstationRefactor2(Writer report, Network network) throws IOException {
		report.write("\tNode '");
		report.write(name);
		report.write("' passes packet on.\n");
	}

	public void workstationRefactor(String s, String workstation, Writer report) throws IOException {
		report.write(s);
		report.write(workstation);
	}

	public void broadcastRefactor(Writer report, Network network, String s) throws IOException {
		report.write("\tNode '");
		report.write(name);
		report.write(s);
	}
